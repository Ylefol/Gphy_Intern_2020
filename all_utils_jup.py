...
def find_diff_increase_spliced_unspliced(vlm,unspli_thresh=0.08,spli_thresh=0.05,upper_unspli_thresh=0.30,bin_size=100, window_size=200):
    """
    A function that goes over the genes in the velocyto dataset and identifies all 
    genes that show a difference between unspliced and spliced during
    an increase in velocity. The difference allowed is dependant on the thresholds
    implemented.
    Thresholds are given as percentages of the total amount of cells in the dataset
    ...
    vlm : velocyto.analysis.VelocytoLoom
        The loom file as read by velocyto.
    unspli_thresh : float, optional
        The percentage of cells in a row that need to be increasing while spliced is
        decreasing for the gene to be considered 'valid'. The default is 0.08.
    spli_thresh : float, optional
        The percentage of cells in a row that will have to exhibit spliced increase
        right after the spliced expression increases (after the unspliced has increased).
        The default is 0.05.
    upper_unspli_thresh : float, optional
        The maximum percentage of cells in a row where unspliced can increase
        If the unspliced expression is always icnreasing, it is likely to not
        be a gene of interest. The default is 0.30.
    bin_size : int, optional
        The bin size used for the calculation of the smoother data.  
        The default is 100.
    window_size: int, optional
        The size of the window used in the moving average for data smoothing.
        The default is 200.
    ...
    Returns
    -------
    good_boy_dict : dictionnary
        dictionnary containing all genes sorted in either 'predict' or 'others'
        the 'predict' key represents genes that have shown to have a significant
        difference between the increase of unspliced and increase of spliced.

    """
    
    spli_thresh=int(len(vlm.ca['CellID'])*spli_thresh)
    unspli_thresh=int(len(vlm.ca['CellID'])*unspli_thresh)
    upper_unspli_thresh=int(len(vlm.ca['CellID'])*upper_unspli_thresh)
    

    good_boy_dict={}
    good_boy_dict['predict']=[]
    good_boy_dict['others']=[]
    ...
    for gene_name in vlm.ra["Gene"]:
        ...
        #Now we are only proceeding with start point that have passed the upper threshold.
        if len(gap_dict['start'])>0:
            
            #upper thresh check
            for idx,val in enumerate(gap_dict['start']):
                if val in index_check_unspli[0]:#start point is good
                    check_list=np.roll(index_check_unspli,-np.where(index_check_unspli==val)[1][0])[0]
                    inc=0
                    check_passed=False
                    for dex,value in enumerate(check_list):
                        if dex==len(check_list)-1:#end of list
                            break                        
                        if value+1==check_list[dex+1]:
                            inc+=1
                            if inc>=upper_unspli_thresh:
                                check_passed=True
                                break
                        else:
                            inc=0
                    if check_passed==True:#The start point is not valid, must be removed
                        gap_dict['start'].pop(idx)
                        gap_dict['end'].pop(idx)
                        continue
            ...
            for idx,val in enumerate(gap_dict["start"]):
                ...
                if gap_dict["end"][idx]<val:#It means we went full circle
                    end_val=np.max(good_indices)+gap_dict["end"][idx]
                else:
                    end_val=gap_dict['end'][idx]
                if abs(val-end_val)>=unspli_thresh:
                    spli_indices.append(gap_dict['end'][idx])
        else:
            good_boy_dict['others'].append(gene_name)
            continue
        if len(spli_indices)>0:#not empty
            #Here we have spli indices that need to confirm that the next n-amount from the i's in the list extend to at least 100
            index_check_positive_spli=np.where(symbols_spli>0)
            for idx,val in enumerate(spli_indices):
                if val+1 in index_check_positive_spli[0]:#start point is good
                    check_list=np.roll(index_check_positive_spli,-np.where(index_check_positive_spli==val+1)[1][0])[0]
                    inc=0
                    check_passed=False
                    for dex,value in enumerate(check_list):    
                        if dex==len(check_list)-1:#end of list
                    ...
                    if check_passed==True:
                        good_boy_dict['predict'].append(gene_name)
                        break
                if idx==len(spli_indices)-1:#End of list
                    good_boy_dict['others'].append(gene_name)
        else:
            good_boy_dict['others'].append(gene_name)
        # break
    return good_boy_dict


def find_diff_decrease_spliced_unspliced(vlm,spli_thresh=0.01,cat_bin=50,unspli_thresh=0.05,bin_size=100, window_size=200):
    """
    Function that sorts all genes into a 'decay category'. These categories represent
    the length/durantion of a difference in the decrease between spliced and unspliced 
    expressions.
    ...
    vlm : velocyto.analysis.VelocytoLoom
        The loom file as read by velocyto.
    spli_thresh : float, optional
        The percentage of cells spliced expression in a row required for an area of a gene
        to be considered 'decaying'. The default is 0.01.
    cat_bin : float, optional
        The bin system used to create decay categories. Genes are fitted in keys
        which represent multiples of the bin (50, 100, 150 etc...). 
        The default is 50.
    unspli_thresh : float, optional
        The precentage of cells unsplcied expression in a row necessary for an area
        of a gene to be considered 'decaying;. The default is 0.05.
    bin_size : int, optional
    ...
    -------
    good_boy_dict : dictionnary
        A dictionnary will all genes sorted in a 'decay category' which are the
    ...
    for gene_name in vlm.ra["Gene"]:
        ...
        for idx,val in enumerate(gap_dict["start"]):
            if gap_dict["end"][idx]<val:#It means we went full circle
                ...
                end_val=gap_dict['end'][idx]
            if abs(val-end_val)>=spli_thresh:
                future_indices.append(gap_dict['end'][idx]+1)
                #Prep category
                category=(int(abs(val-end_val)/cat_bin)+1)*cat_bin
        
        if len(future_indices)==0:
            good_boy_dict['others'].append(gene_name)
            continue        
        index_check_spli_negative=np.where(symbols_spli<0)
        for idx,val in enumerate(future_indices):
            if val+1 in index_check_spli_negative[0]:#start point is good
                check_list=np.roll(index_check_spli_negative,-np.where(index_check_spli_negative==val+1)[1][0])[0]
                inc=0
                check_passed=False
                for dex,value in enumerate(check_list):    
                    if dex==len(check_list)-1:#end of list
                    ...
                    if value+1==check_list[dex+1]:
                        inc+=1
                        if inc==unspli_thresh:
                            check_passed=True
                            break
                    else:
                        inc=0
                if check_passed==True:
                    if str(category) not in good_boy_dict:
                        good_boy_dict[str(category)]=[gene_name]
                    else:
                        good_boy_dict[str(category)].append(gene_name)
                    break
            if idx==len(future_indices)-1:#End of list
                good_boy_dict['others'].append(gene_name)
        # break
    return good_boy_dict
...
def sort_the_predict(predict_dict,decay_dict):
    """
    Function that merges predict and decaying genes to create a dictionnary
    which sorts the predicted increase genes into their decay categories
    
    Function written by Yohan Lefol

    Parameters
    ----------
    predict_dict : dictionnary
        Dictionarry containing all of the 'predict' gene names.
    decay_dict : dictionnary
        Dictionnary containing all of the gene names in their respective
        decay categories.

    Returns
    -------
    predict_decay_dict : dictionnary
        Dictionnary that shows all of the predict genes in their respective
        decay category.

    """
    predict_decay_dict={}
    for k,v in decay_dict.items():
        for gene in v:
            if gene in predict_dict['predict']:#Found one
                if k not in predict_decay_dict:
                    predict_decay_dict[k]=[gene]
                else:
                    predict_decay_dict[k].append(gene)
    return predict_decay_dict


def sort_uniform_genes(vlm,upper_gap_thresh=0.08,bin_size=100, window_size=200):
    """
    Function that finds all of the 'uniform' genes in a velocyto dataset.
    Uniform genes are genes that show no significant difference between spliced and
    unspliced expression at any time point.

    Parameters
    ----------
    vlm : velocyto.analysis.VelocytoLoom
        The loom file as read by velocyto.
    upper_gap_thresh : float, optional
        Precentage of cells in a row representing the maximum alotted difference
        between spliced and unspliced expression. The default is 0.08.
    ...
    -------
    good_boy_dict : dictionnary
        Dictionnary containing all gene names, genes which are 'uniform' are
        stored in the 'uniform' key, all other gene names are stored as 'others'.

    """
    
    upper_gap_thresh=int(len(vlm.ca['CellID'])*upper_gap_thresh)
    
    good_boy_dict={}
    good_boy_dict['uniform']=[]
    good_boy_dict['others']=[]
    orientation=np.unique(vlm.ca['orientation'])[0]
    for gene_name in vlm.ra["Gene"]:
        # gene_name='BIRC5'
        ...
        good_indices=np.where(symbols_spli==symbols_unspli)[0].tolist()
        gap_dict=find_gaps(vlm,good_indices,1)
        #If no gaps are found it means the velocity adds up all the time
        if len(gap_dict['start'])==0:
            good_boy_dict['uniform'].append(gene_name)
            # continue
        ...
        for idx,val in enumerate(gap_dict["start"]):
            if gap_dict["end"][idx]<val:#It means we went full circle
                ...
                end_val=abs(np.max(good_indices)-gap_dict['end'][-1])
            else:
                end_val=gap_dict['end'][idx-1]
            if abs(val-end_val)<=upper_gap_thresh:
                gene_check+=1
                if idx>0:
                    beg_point=gap_dict['end'][idx-1]

                    if symbols_spli[beg_point+1]!=symbols_spli[beg_point]:
                        changed=symbols_spli
                        unchanged=symbols_unspli
                    elif symbols_unspli[beg_point+1]!=symbols_unspli[beg_point]:
                        changed=symbols_unspli
                        unchanged=symbols_spli
                    else:#First iteration/no differences
                        continue
                    if unchanged[val+1]!=changed[val-1]:
                        second_check=False

        if gene_check==len(gap_dict['start']) and second_check==True:  #All checks passed
            good_boy_dict['uniform'].append(gene_name)
        else:
            good_boy_dict['others'].append(gene_name)
...
def grade_gene_sorting(sorted_dict, gene_control_dict, uniform_check=False):
    """
    A grading function to check if the categories in which genes have been
    associated match a control list.

    Parameters
    ----------
    sorted_dict : dictionnary
        The dictionnary that will be graded.
    gene_control_dict : dictionnary
        The control dictionnary.
    uniform_check : boolean, optional
        A boolean which affects which key is used to grade, either 'preditct',
        or 'uniform' depending on the dictionnary inputted. The default is False.
    ...
    if uniform_check==True:
        key='uniform'
    else:
        key='predict'
    for gene in sorted_dict[key]:
        if gene in gene_control_dict['Increase_spliced_predicted']:
            inc_predict+=1
        if gene in gene_control_dict['Uniform_gene']:
            inc_uni+=1
        if gene in gene_control_dict['Steady_gene']:
            inc_steady+=1
        if gene in gene_control_dict['Crossover_genes']:
            inc_cross+=1
        if gene in gene_control_dict['Odd']:
            inc_odd+=1

    print("Total predict found: "+str(len(sorted_dict[key])))
    print(str(inc_predict)+"/"+str(len(gene_control_dict['Increase_spliced_predicted']))+" were predict genes")
    print(str(inc_uni)+"/"+str(len(gene_control_dict['Uniform_gene']))+" were uniform genes")
    print(str(inc_steady)+"/"+str(len(gene_control_dict['Steady_gene']))+" were steady genes")
    print(str(inc_cross)+"/"+str(len(gene_control_dict['Crossover_genes']))+" were crossover genes")
    print(str(inc_odd)+"/"+str(len(gene_control_dict['Odd']))+" were odd genes")
...
def MWU_results(the_dict):
    """
    Simple function which runs the MWU statistical tests on all possibilities
    of a dictionnary resulting from the target scan analysis. The dictionnary
    ...
    ----------
    the_dict : dictionnary
        dictionnary containing numeric values.
    ...
    """
    print('\n Increase + Decrease vs No difference')
    print(stats.ranksums(the_dict['Increase + Decrease'],the_dict['No difference']))
    print('\n Increase only vs No differencel')
    print(stats.ranksums(the_dict['Increase only'],the_dict['No difference']))
    print('\n Increase + Decrease vs Increase only')
    print(stats.ranksums(the_dict['Increase + Decrease'],the_dict['Increase only']))
    merged_list=the_dict['Increase + Decrease']+the_dict['Increase only']
    print('\n merged vs No difference')
    print(stats.ranksums(merged_list,the_dict['No difference']))
...
def MWU_version_2(the_dict):
    """
    Function which generates a MWU statistical test for all keys of a dictionnary
    ...
    ----------
    the_dict : dictionnary
        dictionnary with keys as categories and values as numbers.

    Returns
    -------
    result_dict : dictionnary
        A dictionnary with the pvalue of each MWU combination.

    """
    result_dict={}
    dict_list=list(the_dict.keys())
    for idx,key in enumerate(dict_list):
        temp_key_list=dict_list[idx+1:]
        if len(temp_key_list)==0:
            break
        else:
            for second_key in temp_key_list:
                my_stat_val=stats.ranksums(the_dict[key],the_dict[second_key])
                result_dict[str(key)+" vs "+str(second_key)]=my_stat_val.pvalue
...
def wrapper_function(miRNA_paths,dataset_paths,TargetScan_path,col_names):
    """
    A wrapper function which creates a dataframe of pvalues for an entire
    TargetScan analysis.
    Function iterates over miRNA_paths, then dataset paths.
    For each dataset it will produce a TargetScan analysis using the number of
    target sites per gene as well as the sum of weighted context score.
    These two types are kept separate and only merged at the end.
    
    Therefore the dataframe format will be
    number of target sites for each miRNA path for each dataset path
    THEN
    sum of weighted context score for each miRNA path for each dataset path
    
    The col_name list should reflect this format.
    ...
    ----------
    miRNA_paths : list
        list of file paths for miRNA lists to use in TargetScan analysis.
    dataset_paths : list
        list of file paths for datasets to be used in TargetScan analysis.
    TargetScan_path : string
        file path to the TargetScan prediction file.
    col_names : list
        list of column names for the dataframe produced.

    Returns
    -------
    my_df : pandas.core.frame.DataFrame
        The dataframe containing the pvalues from the MannWhitneyWilcoxon test
        for each of the category combinations presented.

    """
    
    TS_file=pd.read_csv(TargetScan_path,sep='\t')
    
    TS_file=TS_file[TS_file['Gene Tax ID']==9606]
    
    num_ts_dict_list=[]
    sum_weight_dict_list=[]
    for miR in miRNA_paths:
        miRNA_pd=pd.read_csv(miR)
        miRNA_list=list(miRNA_pd.found)
        TS_file=TS_file[TS_file['miRNA'].isin(miRNA_list)]
        print(type(TS_file))
        for data in dataset_paths:
            my_delay_df=pd.read_csv(data)
            gene_dict={}
            gene_dict['increase_delay']=[x for x in list(my_delay_df.inc_gene_names) if str(x) != 'nan']
            gene_dict['decrease_delay']=[x for x in list(my_delay_df.dec_gene_names) if str(x) != 'nan']
            gene_dict['no_increase']=[x for x in list(my_delay_df.no_delay_inc_gene_names) if str(x) != 'nan']
            gene_dict['no_decrease']=[x for x in list(my_delay_df.no_delay_dec_gene_names) if str(x) != 'nan']
            gene_dict['both']=[x for x in gene_dict['increase_delay'] if x in gene_dict['decrease_delay']]
            gene_dict['neither']=[x for x in gene_dict['no_increase'] if x in gene_dict['no_decrease']]

            TS_dict,TS_weight_dict,sum_weight_dict=target_scan_analysis_version_2(TS_file,gene_dict=gene_dict)
            
            target_site_dict=convert_gene_to_numbers(TS_dict)

            temp_num_ts=MWU_version_2(target_site_dict)
            num_ts_dict_list.append(temp_num_ts)
            
            temp_sum_weight=MWU_version_2(sum_weight_dict)
            sum_weight_dict_list.append(temp_sum_weight)
            
    final_dict_list=num_ts_dict_list+sum_weight_dict_list
    final_dict=merge_dicts(final_dict_list)
    
    my_df=pd.DataFrame.from_dict(final_dict,orient='index',columns=col_names)
